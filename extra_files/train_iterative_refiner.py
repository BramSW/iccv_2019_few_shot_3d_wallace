...
point_set_results = {"airplanes":0.601,
                       "benches":0.550,
                       "cabinets":0.771,
                       "cars":0.831,
                       "chairs":0.544,
...
r2n2_single_results = {"airplanes":0.513,
                            "benches":0.421,
                            "cabinets":0.716,
                            "cars":0.798,
                            "chairs":0.466,
...
r2n2_5view_results = {"airplanes":0.561,
                       "benches":0.527,
                       "cabinets":0.772,
                       "cars":0.836,
                       "chairs":0.550,
...
train_categories = copy(all_categories) # ["airplanes"]#, "cars", "chairs", "displays", "phones", "speakers", "tables"]
...
def build_and_train_refiner(load_encoder=True, image_code_dim=16, vox_code_dim=32,
    ...
    if load_encoder:
        ...
        im_ae = load_model('image_ae.h5')
        vox_ae = load_model('voxel_ae.h5')
    else:
    ...
    train_generator = sampler.triple_generator(category_list=train_categories, n_per_yield=batch_size, mode="train")
    
    one_off_train_generator = sampler.triple_generator(category_list=train_categories, n_per_yield=num_test_points, mode='train')
    ...
    while num_epochs_performed<num_epochs:
        ...
        for batch_i in range(batches_per_epoch):
            ...
            for iter_i in range(num_refine_iters):
                vox_ref.refiner.fit([input_images, input_voxels], target_voxels, steps_per_epoch=1, epochs=1, verbose=0)
