...
try:
    results_dict = pickle.load(open("finetuning_results_dict.pickle", "rb"))
except:
    results_dict = {}
for category in categories:
    for num_shots in num_shots_arr:
        ...
        print(category, num_shots)
        performances = []
        for trial_i in range(num_trials):
            print(trial_i)
            baseline_model = keras.models.load_model('/data/bw462/3d_recon/models/baseline.h5')
        ...
        print(results_dict)
        pickle.dump(results_dict, open("finetuning_results_dict.pickle", "wb"))
