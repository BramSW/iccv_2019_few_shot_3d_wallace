...
def iteratively_test_model(model_path_list, num_iters=3, threshold=0.4, num_test_points=1000, average_prior=False,
    ...
    model_list = [models.load_model(model_path) for model_path in model_path_list]
    # print(model.summary())
    for category in all_categories:
        print(category)
        print("FIRST (0th) MODEL BETTER BE BASELINE")
        ...
        all_ious = [[ [] for _ in range(num_iters)] for _ in range(len(model_path_list)) ]
        for batch_i in range(num_batches):
            batch_ious = []
            [input_im, input_vox], target_vox = next(generator)
            model_inputs = [copy(input_vox) for _ in model_list]
            for iter_i in range(num_iters):
                for model_i,model,input_vox in zip(range(len(model_list)), model_list, model_inputs):
                    if model_i == 0: input_vox = np.zeros(input_vox.shape)
                    preds = model.predict([input_im, input_vox])
                    model_inputs[model_i] = preds
                    iou_scores = iou_list(preds, target_vox, threshold=threshold)
                    all_ious[model_i][iter_i].extend(iou_scores)
...
def main():
    ...
    for threshold in thresholds:
        results = iteratively_test_model(args.model_paths.split(','),
    ...
    save_name = input("Save name? (will be appended with .pickle)")
    pickle.dump(results, open(save_name + '.pickle', 'wb'))
