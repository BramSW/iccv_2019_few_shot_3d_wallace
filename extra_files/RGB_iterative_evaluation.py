...
parser.add_argument('--num-test-points', type=int, default=np.inf)
...
def iteratively_test_model(model_path, num_iters=3, threshold=0.4, num_test_points=np.inf, average_prior=False,
    ...
    for category in all_categories:
        ...
        for [input_im, input_vox], target_vox in generator:
            ...
            batches_completed += 1
            if batches_completed == num_batches:
                break
...
def main():
    ...
    cat_group_string = 'transfer' if args.transfer_cats else ('gy' if args.gy_cats else 'all')
    if not args.k_shot_prior:
        pickle_save_string = "test_performances/{}_cats_testing_peformance_".format(cat_group_string) + args.model_path.split('/')[-1].split('.')[0] + ".pickle"
    else:
        pickle_save_string = "test_performances/{}_cats_testing_peformance_{}_shot_".format(cat_group_string, args.k_shot_prior) + args.model_path.split('/')[-1].split('.')[0] + ".pickle" 
