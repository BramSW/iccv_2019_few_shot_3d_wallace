...
def build_and_train_refiner(load_encoder=True, image_code_dim=512, vox_code_dim=512,
    ...
    train_generator = highres_sampler.multiview_triple_generator(category_list=all_categories, n_per_yield=batch_size, mode="train", input_average_voxel=average_prior, k_shot_prior=k_shot_prior, num_ims=num_refine_iters)
    
    one_off_train_generator = highres_sampler.multiview_triple_generator(category_list=all_categories, n_per_yield=num_test_points, mode='train', input_average_voxel=average_prior, k_shot_prior=k_shot_prior, num_ims=num_refine_iters)
    ...
    for category in all_categories:
        ...
        cat_generator = highres_sampler.multiview_triple_generator(category_list=[category], n_per_yield=num_test_points, mode="val", 
                                                         input_average_voxel=average_prior, k_shot_prior=k_shot_prior,
                                                         num_ims=num_refine_iters)
    ...
    while num_epochs_performed<num_epochs:
        ...
        if num_refine_iters != 1:
            for batch_i in range(batches_per_epoch):
                ...
                for iter_i in range(num_refine_iters):
                    vox_ref.refiner.fit([input_images[:,iter_i,:,:,:], input_voxels], target_voxels, epochs=1, 
                                        verbose=args.verbosity)
                    input_voxels = vox_ref.refiner.predict([input_images[:,iter_i,:,:,:], input_voxels])
        ...
        for iter_index in range(num_refine_iters):
            score = test_iou4(vox_ref.refiner, train_input_images[:,iter_index,:,:,:], copy_train_input_voxels, train_target_voxels)
            train_scores.append(score)
            copy_train_input_voxels = vox_ref.refiner.predict([train_input_images[:,iter_index,:,:,:], copy_train_input_voxels])
        ...
        for cat_i, category in enumerate(all_categories):
            ...
            for iter_index in range(num_refine_iters):
                score = test_iou4(vox_ref.refiner, cat_input_images[:,iter_index,:,:,:], cat_input_voxels,cat_target_voxels)
                ...
                cat_input_voxels = vox_ref.refiner.predict([cat_input_images[:,iter_index,:,:,:], cat_input_voxels])
